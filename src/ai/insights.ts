...
export function generateInsights(
    ...
    sorted.slice(0, 5).forEach((p, i) => {
        if (p.totalSold > 0) {
            insights.push({
                type: 'top_selling',
                productName: p.name,
                message: `#${i + 1} best seller — ${p.totalSold} units sold (₹${p.revenue.toFixed(0)} revenue)`,
                value: p.totalSold,
            });
        }
    });

    // Bottom 5 (products with some stock but very low/no sales)
    ...
    // For each product with enough data, fit a line to daily sales.
    // A positive slope = trending up, negative = trending down.
    for (const product of products) {
        const dailyData = dailySalesByProduct[product.id];
        if (!dailyData) continue;

        // Build time series: day index → quantity sold
        ...
        // Need at least 7 non-zero days for meaningful trend
        ...
        try {
            ...
            // Skip if regression fails (e.g., insufficient data variance)
            continue;
        }
    }
